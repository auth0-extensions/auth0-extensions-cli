const _ = require('lodash');
const path = require('path');
const semver = require('semver');
const Webpack = require('webpack');
const TerserPlugin = require("terser-webpack-plugin");
const NodePolyfillPlugin = require('node-polyfill-webpack-plugin');

function convertDependenciesToCommonJS(dependencies) {
  return dependencies.reduce((acc, dep) => {
    const versionIndex = dep.lastIndexOf('@');

    if (versionIndex > 0) {
      acc[dep.substring(0, versionIndex)] = `commonjs ${dep}`;
    } else {
      acc[dep] = `commonjs ${dep}`;
    }
    return acc;
  }, {});
}

const excludeModules = list =>
  (modulePath) => {
    if (modulePath.indexOf('node_modules') < 0) {
      return false;
    }

    let isIncluded = true;

    _.each(list, (module) => {
      if (modulePath.indexOf(path.join('node_modules', module)) >= 0) {
        isIncluded = false;
      }
    });

    return isIncluded;
  };

module.exports = (pkg, rootPath, args, ext) => {
  const externals = Object.assign({}, ext);
  const extension = pkg['auth0-extension'] || {};
  const dependencies = pkg.dependencies;
  const mappings = extension.externals || [];
  const excluded = extension.excluded || ['express-conditional-middleware', 'pino'];
  const additionalSettings = _.pickBy(process.env, (val, key) => key.startsWith('A0EXT_') || key === 'PR_NUMBER');
  const settings = _.assign(extension.settings, {
    NODE_ENV: 'production',
    CLIENT_VERSION: pkg.version
  }, additionalSettings);


  console.log({
    "path": path.join(rootPath, args.entryPoint),
    "node_modules_path": path.join(rootPath, './node_modules/')
  })

  // Transform to JSON.
  Object.keys(settings).forEach((k) => {
    settings[k] = JSON.stringify(settings[k]);
  });

  const activePlugins = [];
  const activeLoaders = [];

  let babelExclude;
  if (typeof extension.bundleModules !== 'undefined' && extension.bundleModules === false) {
    externals.compatible = convertDependenciesToCommonJS(Object.keys(dependencies));
    babelExclude = /node_modules/;
  } else {
    // Add custom mappings to externals.
    externals.compatible = convertDependenciesToCommonJS(mappings);
    babelExclude = excludeModules(excluded);
  }


  console.log({
    "babelExclude": babelExclude,
    "externals.compatible": externals.compatible,
    'joi-incompatible': externals.incompatible['joi']
  })

  const ecma = 8;
  const nodeTarget = '18.16.0';

  if (
    typeof extension.useBabel === 'undefined' ||
    extension.useBabel === true
  ) {
    activeLoaders.push({
      test: /\.js$/,
      use: {
        loader: 'babel-loader',
        options: {
          babelrc: false,
          "sourceType": "unambiguous",
          presets: [
            ['@babel/preset-env', {
              targets: {
                node: nodeTarget
              },
              shippedProposals: true
            }]
          ],
          plugins: []
        }
      },
      exclude: babelExclude
    });
  }

  activePlugins.push(
    new Webpack.DefinePlugin({
      'process.env': settings
    })
  );

  // add "use strict" to the very beginning of the bundle to avoid some of node4 issues
  activePlugins.push(
    new Webpack.BannerPlugin({
      banner: '"use strict";',
      raw: true
    })
  );

  activePlugins.push(new NodePolyfillPlugin({
    additionalAliases: ['vm']
  }));
  
  // Return config.
  return {
    entry: path.join(rootPath, args.entryPoint),
    mode: 'none',
    target: 'node',
    output: {
      path: path.join(rootPath, args.destinationFolder),
      filename: `${pkg.name}.extension.${pkg.version}.js`,
      library: {
        type: "commonjs2",
      },
      iife: false
    },
    optimization: {
      minimize: true,
      minimizer: [new TerserPlugin({
        terserOptions: {
          ecma,
          format: {
            comments: false,
          },
          compress: {
            warnings: false,
            unused: true,
            dead_code: true
          },
        }
      })],
    },
    externals: externals.compatible,
    module: {
      rules: activeLoaders
    },
    plugins: activePlugins,
    resolve: {
      modules: [path.join(rootPath, './node_modules/'), 'node_modules'],
      // https://github.com/auth0/node-auth0/issues/657#issuecomment-928083729
      alias: {
        'formidable': false, //  node-auth0 build warning
        'coffee-script': false, //  node-auth0 build fail
        'vm2': false, // node-auth0 build fail
        'yargs': false, // auth0-deploy-cli build warning
        'colors': false, // auth0-deploy-cli build warning
        'keyv': false, // openid-client build warning
        'handlebars' : 'handlebars/dist/handlebars.js'
      }
    }
  };
};
